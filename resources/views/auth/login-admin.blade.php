...
<head>
    ...
    <title>Admin Login - Harsasetialiving</title>
        ...
        <form method="POST" action="{{ route('login.admin.post') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" required 
                    class="w-full bg-gray-800 border {{ $errors->has('email') ? 'border-red-500' : 'border-gray-700' }} rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500"
                    value="{{ old('email') }}">
                @error('email')
