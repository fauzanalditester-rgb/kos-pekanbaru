<div class="space-y-6">
                    ...
                    <tr class="hover:bg-gray-800/30 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @endif
                                <button wire:click="openWhatsAppModal({{ $invoice->id }})" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-500/10 hover:bg-green-500/20 text-green-500 rounded-lg transition-colors text-xs font-medium" title="Hubungi via WhatsApp">
    ...
    <!-- WhatsApp Modal -->
    @if($showWhatsAppModal && $selectedInvoice)
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-[#111827] border border-gray-800/50 rounded-2xl w-full max-w-md">
                <div class="p-6 border-b border-gray-800/50 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-lg font-semibold text-white">Kirim WhatsApp</h3>
                            <p class="text-sm text-gray-400">{{ $selectedInvoice->tenant->name }}</p>
                        </div>
                    </div>
                    <button wire:click="closeWhatsAppModal" class="text-gray-500 hover:text-white transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="bg-gray-800/50 rounded-xl p-4 space-y-2">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="text-white font-mono">{{ $selectedInvoice->invoice_number }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-400">Total</span>
                            <span class="text-white font-semibold">Rp {{ number_format($selectedInvoice->total_amount, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-400">Jatuh Tempo</span>
                            <span class="text-white">{{ $selectedInvoice->due_date->format('d M Y') }}</span>
                        </div>
                    </div>

                    <!-- Phone Info -->
                    <div class="flex items-center gap-3 p-3 bg-green-500/10 border border-green-500/20 rounded-xl">
                        <svg class="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                        </svg>
                        <div>
                            <p class="text-sm text-gray-400">No. Telepon</p>
                            <p class="text-white font-medium">{{ $selectedInvoice->tenant->phone }}</p>
                        </div>
                    </div>

                    <!-- Message Template -->
                    <div>
                        <label class="block text-sm font-medium text-gray-400 mb-2">Kirim Pesan</label>
                        <textarea wire:model="whatsappMessage" rows="6" class="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-green-500 text-sm" placeholder="Tulis pesan WhatsApp..."></textarea>
                    </div>

                    <!-- Actions -->
                    <div class="flex items-center justify-end gap-3 pt-2">
                        <button type="button" wire:click="closeWhatsAppModal" class="px-4 py-2.5 text-gray-400 hover:text-white transition-colors text-sm">
                            Batal
                        </button>
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $selectedInvoice->tenant->phone) }}?text={{ urlencode($whatsappMessage) }}" target="_blank" class="inline-flex items-center gap-2 px-6 py-2.5 bg-green-500 hover:bg-green-600 text-white font-medium rounded-xl transition-colors">
